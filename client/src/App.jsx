...
export default function ChickFarm() {
  ...
  const [analytics, setAnalytics] = useState({
    totals: { totalDeaths: 0, totalFeed: 0, totalExpenses: 0, totalIncome: 0 },
    trends: {
      feed: [],
      deaths: [],
      income: [],
  ...
  const [aiLastRequestAt, setAiLastRequestAt] = useState(0);
  ...
  const loadAnalytics = useCallback(async () => {
    try {
      const [totals, feedTrend, deathsTrend, incomeTrend, expensesTrend, feedVsDeaths] = await Promise.all([
        analyticsService.getTotals(),
        analyticsService.getFeedTrend(),
        analyticsService.getDeathTrend(),
        analyticsService.getIncomeTrend(),
        analyticsService.getExpensesTrend(),
        analyticsService.getFeedVsDeaths(),
      ]);
      const analyticsData = {
        totals,
        trends: {
  ...
  const buildAiPayload = useCallback((analyticsData) => {
    if (!analyticsData) return null;
    const totals = analyticsData.totals || {};
    const mortalityRate = totals.totalDeaths > 0
      ? totals.totalDeaths / Math.max(1, totals.totalDeaths + totals.totalFeed)
      : 0;
    const profit = totals.totalIncome - totals.totalExpenses;
    const deathsTrend = analyticsData.trends.deaths || [];
    const averageDeaths = deathsTrend.length
      ? deathsTrend.reduce((sum, item) => sum + Number(item.value || 0), 0) / deathsTrend.length
      : 0;
    const anomaliesDetected = deathsTrend.filter((item) => Number(item.value || 0) > averageDeaths * 2).length;

    return {
      totals,
      trends: analyticsData.trends,
      feedVsDeaths: analyticsData.feedVsDeaths,
      derivedMetrics: {
        mortalityRate,
        profit,
        anomaliesDetected,
  ...
  const fetchAIInsights = useCallback(async (structuredAnalytics) => {
    if (!structuredAnalytics) return;
    const now = Date.now();
    if (aiLoading) return;
    if (now - aiLastRequestAt < AI_COOLDOWN_MS) {
      setAiError("Please wait before requesting insights again.");
      return;
    }

    setAiLoading(true);
    setAiError(null);
    setAiLastRequestAt(now);
    try {
      const insights = await aiService.getInsights(structuredAnalytics);
      setAiInsights(insights);
    } catch (error) {
      console.error("Failed to fetch AI insights", error);
      setAiError(error.message || "AI insights unavailable");
      setAiInsights(null);
    } finally {
      setAiLoading(false);
    }
  }, [aiLastRequestAt, aiLoading]);

  const triggerAiInsights = useCallback(async () => {
    const analyticsData = await loadAnalytics();
    const payload = buildAiPayload(analyticsData);
    if (payload) {
      await fetchAIInsights(payload);
    }
  }, [loadAnalytics, buildAiPayload, fetchAIInsights]);

  // Load shared records from SQLite and sync with Firestore
  const loadRecords = useCallback(async (organizationId = null) => {
    try {
      ...
      const rows = await dbService.getAllRecords();
      setRecords(rows);
      if (navigator.onLine) {
        const merged = await syncService.sync();
        setRecords(merged);
      }
      return rows;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      try {
        ...
        if (session.organization && session.organization.id !== organization?.id) {
          setOrganization(session.organization);
        }

        await dbService.migrateRecordsToOrganization(session.organization.id);
        await loadRecords(session.organization.id);
        await loadAnalytics();
      } catch (error) {
  ...
  const saveRecords = async (newRecs) => {
    const orgId = organization?.id || currentUser?.organizationId || null;
    const recordsWithOrg = newRecs.map((record) => ({
      ...record,
      organizationId: record.organizationId || orgId,
    }));
    setRecords(recordsWithOrg);
    await dbService.saveAllRecords(recordsWithOrg);
    await loadAnalytics();
    if (navigator.onLine) {
      syncService.sync().then(async (merged) => {
        setRecords(merged);
        await loadAnalytics();
      }).catch(console.error);
    }
  ...
  const addRecord = async (rec) => {
    const orgId = organization?.id || currentUser?.organizationId || null;
    const newRecs = [{ ...rec, id: uid(), addedBy: currentUser?.name || rec.addedBy || "Unknown", organizationId: orgId, createdAt: Date.now() }, ...records]
  ...
  const totals = useMemo(() => {
    ...
    return {
      income: analyticsTotals.totalIncome || 0,
      expenses: analyticsTotals.totalExpenses || 0,
      deaths: analyticsTotals.totalDeaths || 0,
      injured: records.reduce((a, r) => a + (Number(r.injured) || 0), 0),
      sold: records.reduce((a, r) => a + (Number(r.soldBirds) || 0), 0),
      profit: (analyticsTotals.totalIncome || 0) - (analyticsTotals.totalExpenses || 0),
    };
  }, [analytics.totals, records]);
    ...
    <div style={S.app}>
      ...
      <div style={S.sidebar}>
        ...
        <div style={S.userBadge}>
          ...
          <div style={{ fontWeight: 600, color: T.straw }}>{currentUser?.name || "—"}</div>
          <div style={{ fontSize: 10, color: "rgba(232,213,163,0.5)", marginTop: 8 }}>ORGANIZATION</div>
          <div style={{ fontWeight: 600, color: T.straw }}>{organization?.name || "Solo Farm"}</div>
          <div style={{ fontSize: 11, color: T.bark, marginTop: 4 }}>Role: {currentUser?.role || "viewer"}</div>
          <div
            onClick={() => setShowUserModal(true)}
            style={{ marginTop: 8, fontSize: 11, color: T.amber, cursor: "pointer" }}>
            Change user →
          </div>
        </div>
      </div>
      ...
      <div style={S.main}>
        {loading ? (
          ...
          <>
            <div style={S.card}>
              <h3 style={S.sectionTitle}>AI Farm Insights</h3>
              <p style={{ color: T.bark, marginBottom: 16 }}>Generate AI-backed commentary from your derived analytics. Press Analyze when ready.</p>
              <div style={{ marginBottom: 16 }}>
                <button style={S.btn()} onClick={triggerAiInsights} disabled={aiLoading}>
                  {aiLoading ? "Analyzing…" : "Analyze Farm Analytics"}
                </button>
              </div>
              {aiError && <div style={{ color: T.rust, marginBottom: 12 }}>AI error: {aiError}</div>}
              {aiInsights && !aiLoading && (
                <div style={{ color: T.brown, lineHeight: 1.6 }}>
                  {aiInsights.summary && <p>{aiInsights.summary}</p>}
                  {aiInsights.anomalies && <p><strong>Anomalies:</strong> {aiInsights.anomalies}</p>}
                  {aiInsights.recommendations && <p><strong>Recommendations:</strong> {aiInsights.recommendations}</p>}
                  <p><strong>Derived metrics:</strong> mortalityRate={aiInsights.mortalityRate}, profit={aiInsights.profit}, anomaliesDetected={aiInsights.anomaliesDetected}</p>
                </div>
              )}
            </div>
            {view === "dashboard" && <Dashboard records={records} totals={totals} latest={latest} last7={last7} setView={setView} />}
            {view === "add"       && <AddRecord onSave={addRecord} userName={currentUser?.name || ""} />}
      ...
      {showUserModal && (
        <UserModal
          ...
          onSave={async (form) => {
            setLoading(true);
            try {
              const session = await authService.loginOrSignup(form);
              setCurrentUser(session.user);
              setOrganization(session.organization);
              ...
              showToast(error.message || "Login/signup failed", "danger");
              return;
            } finally {
              setLoading(false);
            }
...
function UserModal({ current, onSave }) {
  ...
  const handleSave = async () => {
    ...
    await onSave({
      ...
      accountType,
      organizationName: accountType === "organization" ? organizationName.trim() : "",
    });
  };
    ...
    <div style={{
      ...
      <div style={{ ...S.card, width:360, margin:0, textAlign:"center" }}>
        ...
        <p style={{ color:T.bark, fontSize:14, marginBottom:20 }}>
          Sign in with your account, or create a new individual or organization login.
        </p>
        <input
          style={S.input}
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 8, marginBottom: 8 }}>
          <button
            style={{ ...S.btn(accountType === "individual"), flex: 1 }}
            onClick={() => setAccountType("individual")}
          >Individual</button>
          <button
            style={{ ...S.btn(accountType === "organization"), flex: 1 }}
        ...
        {error && <div style={{ color: T.rust, fontSize: 13, marginTop: 6 }}>{error}</div>}
        <button
